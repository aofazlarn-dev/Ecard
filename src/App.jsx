...
export default function App() {
  ...
  return <>
    ...
    <header className="site-header"><a className="brand" href="#home">{w.brideDisplay}<span>&</span>{w.groomDisplay}<small>THE WEDDING</small></a><button className="mobile-toggle" aria-label="เปิดเมนู" aria-expanded={menu} onClick={() => setMenu(!menu)}>{menu ? <X/> : <Menu/>}</button><nav className={menu ? 'open' : ''} onClick={e=>{if(e.target.closest('a'))setMenu(false);}}><a href="#story">คำเชิญจากเรา</a><a href="#invitation">การ์ดเชิญ</a><a href="#prewedding">Pre-wedding</a><a href="#details">รายละเอียดงาน</a><a href="#rsvp" className="nav-rsvp">ตอบรับคำเชิญ <ArrowUpRight size={14}/></a></nav></header>
    <main>
      <WeddingHero/>
      <Countdown/>
      <section id="story" className="story section reveal"><div className="eyebrow">A LITTLE NOTE TO YOU</div><Heart size={25} strokeWidth={1} className="story-heart"/><h2>วันพิเศษของเรา<br/>จะสมบูรณ์ขึ้น เมื่อมีคุณ</h2><div className="hosts">{w.hosts.map(host => <div key={host.name}><span>{host.name}</span><small>{host.role}</small></div>)}</div><p className="invitation">{w.invitation}</p><p className="couple-full-names">{w.brideFullName}<br/><span>&</span><br/>{w.groomFullName}</p><div className="signature">{w.bride} <span>&</span> {w.groom}</div><p className="fine-print invitation-note">{w.invitationNote}</p></section>
      <PreWeddingGallery/>
      <InvitationGallery/>
      <section id="details" className="details section reveal"><div className="section-heading"><div><div className="eyebrow">THE CELEBRATION</div><h2>นัดหมายแห่งความสุข</h2></div><span className="fine-print">เก็บวันนี้ไว้ให้เรานะ</span></div><div className="detail-grid"><article className="detail-card"><CalendarDays/><h3>{w.dateLabel}</h3><p>{w.timeLabel}</p><CalendarLink/></article><article className="detail-card"><MapPin/><h3>{w.venue}</h3><p>{w.address}</p>{w.mapUrl ? <a className="text-link" href={w.mapUrl} target="_blank" rel="noreferrer">ดูแผนที่ <ArrowUpRight size={16}/></a> : <span className="fine-print">แผนที่จะพร้อมเมื่อยืนยันสถานที่</span>}</article><article className="detail-card"><Heart/><h3>Dress code</h3><p>{w.dressCodeLabel}</p><div className="swatches" aria-label={`Dress code ${w.dressCodeLabel}`}>{w.dressCode.map(c => <span key={c} style={{background:c}}/>)}</div></article></div><div className="schedule">{w.schedule.map((item,i)=><div className="schedule-item" key={item.time}><span className="schedule-number">0{i+1}</span><time>{item.time}</time><h3>{item.title}</h3><p>{item.detail}</p></div>)}</div></section>
