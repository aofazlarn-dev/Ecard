...
export default function App() {
  ...
  return <>
    ...
    <main>
      ...
      <section id="story" className="story section reveal"><div className="eyebrow">A LITTLE NOTE TO YOU</div><Heart size={25} strokeWidth={1} className="story-heart"/><h2>วันพิเศษของเรา<br/>จะสมบูรณ์ขึ้น เมื่อมีคุณ</h2><div className="family-invitation"><div className="hosts">{w.hosts.map(host => <div key={host.name}><span>{host.name}</span><small>{host.role}</small></div>)}</div><p className="invitation">{w.invitation}</p><p className="couple-full-names">{w.brideFullName}<br/><span>&</span><br/>{w.groomFullName}</p></div><div className="signature">{w.bride} <span>&</span> {w.groom}</div><p className="fine-print invitation-note">{w.invitationNote}</p></section>
      <PreWeddingGallery/>
      <InvitationGallery/>
